...
```text
release/v0.1/agent-runtime-0.1.0-py312-pyc-only.tar.gz
```
...
sha256sum -c agent-runtime-0.1.0-py312-pyc-only.tar.gz.sha256  # 校验失败必须停止安装
...
替换 `release/v0.1/` 下 tar.gz 后必须同步更新 `*.tar.gz.sha256`：

```bash
cd vendor/agent-runtime/release/v0.1
shasum -a 256 agent-runtime-0.1.0-py312-pyc-only.tar.gz \
  | awk '{print $1"  agent-runtime-0.1.0-py312-pyc-only.tar.gz"}' \
  > agent-runtime-0.1.0-py312-pyc-only.tar.gz.sha256
```
