...
```text
release/v1.1.3/agent-runtime-1.1.3-py312-pyc-only.tar.gz
```
...
```bash
cd /path/to/zw-brain/vendor/agent-runtime/release/v1.1.3
sha256sum -c agent-runtime-1.1.3-py312-pyc-only.tar.gz.sha256  # 校验失败必须停止安装
...
替换 `release/v1.1.3/` 下 tar.gz 后必须同步更新 `*.tar.gz.sha256`：

```bash
cd vendor/agent-runtime/release/v1.1.3
shasum -a 256 agent-runtime-1.1.3-py312-pyc-only.tar.gz \
  | awk '{print $1"  agent-runtime-1.1.3-py312-pyc-only.tar.gz"}' \
  > agent-runtime-1.1.3-py312-pyc-only.tar.gz.sha256
```
