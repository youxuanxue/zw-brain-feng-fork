...
def _register_capability_package(brain, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def mutation(audit_id: str, actor: str) -> dict[str, Any]:
        ...
        brain._append_audit_feed("capability.package.register", package_id, "ok", actor)
        return {"package_id": package_id, "slug": slug, "status": package_payload["status"], "audit_id": audit_id}

    return brain._mutate("capability.package.register", role, confirmed, payload, mutation)

def _register_package_version(brain, package_id: str, role: str, confirmed: bool, skill_id: str = "package.register_version") -> dict[str, Any]:
    ...
    def mutation(audit_id: str, actor: str) -> dict[str, Any]:
        ...
        brain._append_audit_feed(skill_id, package_id, "ok", actor)
        return {"package_id": package_id, "registered_version": item["registeredVersion"]}

    return brain._mutate(skill_id, role, confirmed, {"package_id": package_id}, mutation)

def _review_package(brain, package_id: str, decision: str, role: str, confirmed: bool, skill_id: str = "package.review_decide") -> dict[str, Any]:
    item = brain._package_by_id(package_id)
    ...
    def mutation(audit_id: str, actor: str) -> dict[str, Any]:
        if decision == "approve":
            ...
            brain._append_audit_feed("package.approve", package_id, "ok", actor)
        elif decision == "return_for_fix":
            item["status"] = "pending-fix"
            item["aiReview"]["summary"] = "该能力包需要先补齐租户范围或 side effects 声明，当前不进入上线。"
            brain._append_audit_feed("package.return-for-fix", package_id, "warning", actor)
        elif decision == "reject":
            item["status"] = "rejected"
            item["aiReview"]["summary"] = "该能力包因越界写权或暴露面设计不合规被驳回。"
            item["aiReview"]["draft"] = "审核结论：驳回。请回到单一契约并撤销越界写权声明后再重新提交。"
            brain._append_audit_feed("package.reject", package_id, "warning", actor)
        else:
            raise BrainServiceError(f"unsupported package decision: {decision}")
        return {"package_id": package_id, "status": item["status"]}

    return brain._mutate(skill_id, role, confirmed, {"package_id": package_id, "decision": decision}, mutation)

def _configure_package_exposure(brain, package_id: str, mode: str, role: str, confirmed: bool, skill_id: str = "package.configure_exposure") -> dict[str, Any]:
    item = brain._package_by_id(package_id)
    ...
    return brain._mutate(skill_id, role, confirmed, {"package_id": package_id, "mode": mode}, mutation)

def _apply_package_tenant_policy(brain, package_id: str, role: str, confirmed: bool, skill_id: str = "package.apply_tenant_policy", tenant_id: str = _DEFAULT_TENANT_ID) -> dict[str, Any]:
    item = brain._package_by_id(package_id)
    if item.get("versionStatus") != "registered":
    ...
    return brain._mutate(skill_id, role, confirmed, {"package_id": package_id, "tenant_id": tenant_id}, mutation)

def _list_packages(brain) -> list[dict[str, Any]]:
...
def _get_package(brain, package_id: str) -> dict[str, Any]:
...
def _disable_tenant_capability(brain, package_id: str, role: str, confirmed: bool, tenant_id: str = _DEFAULT_TENANT_ID) -> dict[str, Any]:
    item = brain._package_by_id(package_id)
    ...
    def mutation(audit_id: str, actor: str) -> dict[str, Any]:
        ...
        brain._append_audit_feed("tenant.capability.disable", package_id, "ok", actor)
        return {"package_id": package_id, "tenant_policy_status": item["tenantPolicy"]["policyStatus"], "tenant_id": tenant_id, "audit_id": audit_id}

    return brain._mutate("tenant.capability.disable", role, confirmed, {"package_id": package_id, "tenant_id": tenant_id}, mutation)
