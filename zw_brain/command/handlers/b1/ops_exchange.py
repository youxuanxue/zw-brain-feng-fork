...
def _diagnose_exchange(brain, *, task_id: Any = None, attempt_id: Any = None) -> dict[str, Any]:
    ...
    attempts = [brain._delivery_attempt_record_to_dict(item) for item in brain._delivery_repo().list_attempts(delivery_code=delivery_code, attempt_code=attempt_code)]
    evidence = [brain._delivery_evidence_record_to_dict(item) for item in brain._delivery_repo().list_execution_evidence(delivery_code=delivery_code, attempt_code=attempt_code, tenant_id=_DEFAULT_TENANT_ID)]
    metrics = [brain._exchange_metric_record_to_dict(item) for item in brain._delivery_repo().list_exchange_metrics(delivery_code=delivery_code, tenant_id=_DEFAULT_TENANT_ID)]
