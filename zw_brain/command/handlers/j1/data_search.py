...
def _recall_candidates(deps: Any, haystack: str, seen_ids: set[str], org_name: Any | None = None) -> list[dict[str, Any]]:
    ...
    for entry in recall.get("sample_titles", []):
        ...
        if record is not None:
            ...
            card = deps.services.catalog.record_to_card_dict(record, org_name_resolver=org_name)
        ...
        out.append(
            {
                "id": cand_id,
                "name": title,
                "provider": str(org_name(entry.get("owner_org_id")) if org_name is not None else "") or "—",
                "zone": "官方目录推荐",
                "status": "录入中",
...
def _active_catalog_cards(
    ...
    return [
        deps.services.catalog.record_to_card_dict(record, org_name_resolver=org_name)
        | {"topicProjections": cards_by_catalog.get(record.catalog_code, [])}
        for record in records
    ]


def _extend_catalog_records_by_org_query(deps: Any, records: list[Any], query: str) -> list[Any]:
    """data.search 支持按机构名/机构码搜目录：query→org_projection→owner_org_id 补查。"""
    org_code = ReferenceService().resolve_org_code(query, tenant_id=_DEFAULT_TENANT_ID)
    if not org_code:
        return records
    by_code = deps.repos.catalog.list_entries(
        tenant_id=_DEFAULT_TENANT_ID,
        lifecycle_status="active",
        owner_org_id=org_code,
        exclude_catalog_code_prefix="api-group:",
    )
    seen = {str(record.catalog_code) for record in records}
    out = list(records)
    for record in by_code:
        code = str(record.catalog_code)
        if code not in seen:
            seen.add(code)
            out.append(record)
    return out
...
def search_resources(brain: BrainService, query: str, page: int = 1) -> dict[str, Any]:
    ...
    if store is None:
        ...
        def org_name(_value: str) -> str:
            return ""  # 内存 fallback 无 org_projection；不查 DB、不展示裸机构码。
        ...
        for api_res in deps.view.resources.list_api_resources():
            ...
            provider = org_name(api_res.get("owner_org_id"))
            text = " ".join(
                [
                    api_res.get("title", ""),
        ...
        if haystack:
            ...
            resources.extend(_recall_candidates(deps, haystack, seen_ids, org_name))
    else:
        if not query:
            ...
            records = _extend_catalog_records_by_org_query(deps, records, query)
            ...
            resources = _active_catalog_cards(deps, records, store, org_name)
            ...
            resources.extend(_recall_candidates(deps, haystack, existing_ids, org_name))
    page = max(page, 1)
    page_size = 20
