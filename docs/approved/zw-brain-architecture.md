...
status: approved
gate: approved
approved_by: xuejiao02 + 海若产品部业务方
...
> **设计哲学**：以乔布斯产品哲学（聚焦、简洁、端到端体验、精品意识）与确定性自动化运营和运维原则（杠杆最大化、流程极简、自动化优先）为收敛准绳。
> **配对文档**：本文回答"造什么样的产品"；[`zw-brain-flywheel.md`](zw-brain-flywheel.md) 回答"怎么持续把 N 个客户上线"（业务提升飞轮设计），两者正交、不重复。
...
**zw-brain 是一个让政府部门之间共享数据的产品——"数据共享平台"。**

> 对外/对客户的一句话（灵魂叙事，F/D60）：**"要数据，不用再跑窗口、不用再问我是哪个角色、拿到就能调用。"**
> "数据共享平台"是工程/架构内部品类口径；用户感知的是上面这句体验承诺（不再用管道工口吻的"网关"对外讲）。
...
项目代号 `zw-brain`（"政务数据大脑"）继续沿用；产品身份是"数据共享平台"（对外灵魂叙事见 §1.1 顶部体验承诺句，F/D60）。
...
| 人口库 / 法人库（线下建库类） | 各部门线下建库 + 数据治理中心治理（**D31/D32 修订**：主题库 / 专题库 / 数购车业务方 PR #129 复活，按 `docs/reconstructs/dsp-sharezone-topic-package-reconstruction-plan-v1.md` 落地 Wave 2 P7；详见 §5.6 #11）|
| 运行监控 / 告警 / 巡检 | 集团统一运维监控平台 |
| 国家目录治理 / 国家直达 | 客户重心在本级共享 |
| 数据存证 / 区块链 | 可插拔 adapter；外链 down 不阻塞业务 |
| 标准服务 / 指标平台 / 应用案例 | 旧平台"几乎不用" |
| 脚本管理 / 指标平台 / 绩效考核 | 旧平台"几乎不用"，业务方 PR #129 B/C/H 类签字接受不复刻 |
| 通用服务 / 融合服务编排 / 代理服务 | **D31/D32 修订**：业务方 PR #129 A 类复活 20/21，按 `docs/reconstructs/dsp-dataservice-reconstruction-plan-v1.md` 落地（散落到 Wave 0/1/2/3）；106 案例提出页保持不复活 |
...
### 2.2 确定性自动化运营和运维：可确定性协同运转，才有资格成为默认方案
...
| 5 | **客户差异通过 R14 三引擎 + 多租户策略 + 外部能力包**承接，**不通过 fork 后端** | 立项会议业务方原话"各项目门户、表单、流程都个性化；旧平台靠改代码 + 改数据库" |
...
**admin 旅程占位退役**：WebUI 不再保留非主旅程空占位路由；迁移状态核验只保留后端只读查询与部署 runbook，不挂独立页面。
...
| 应用中心 / 应用案例 | app-center-web | 2 | **§5.6 #11 #12.a 不进 IA**（H 类业务方接受不复刻）| ❌ 不复造 |
...
**不进 IA 清单**（业务方 sign-off + 基于旧库真实分布）：

> **2026-05-27 业务方 PR #129 sign-off 触发修订（D31）**：#11 #12 部分**复活**（A 类 20 条服务能力面 + D 类 4 条主题库），归属 Wave 2.x+ 待立项；非简单复刻旧实现，业务方明确"不需要 BSP / 不需要旧门户 / 整合后重新设计"。详见 [docs/legacy-not-reproduce-signoff.md](../legacy-not-reproduce-signoff.md)。
...
| 12 | **融合 / 通用 / 代理服务**（21 条 → A 类）| **D31/D32 修订**：业务方 PR #129 复活 20/21 条（106 案例提出页除外），按 `docs/reconstructs/dsp-dataservice-reconstruction-plan-v1.md` 落地（Wave 0 网关投影 / Wave 1 API 资源化 / Wave 2 R14 审批 / Wave 3 编排外部化）；业务方明确"整合后重新设计为服务能力面，不需要 BSP / 旧门户" | PR #129 A 类签字 + D32 |
| 12.a | 应用中心 / 应用案例（H 类 13 条） | 业务方 PR #129 接受不复刻 | 业务方 PR #129 H 类签字 |
| 13 | 基础主题库（`dsp_basesubject` 81 张独立表） | 不复造；归外部数据治理中心或客户线下编制 | 旧平台真实存在的并行编制系统，与政务目录平行（基于 `dsp_basesubject.xml` 81 表事实） |

**复活分支约束（D31.c）**：复活的"服务能力面"与"主题库"必须走 R8 反 per-tenant fork + R14 三引擎，禁止为复活功能在 zw-brain 内部复造旧 BSP / 旧门户结构；preflight 段 22 (capability-boundary) + 段 25 (adapter-write-ban) 继续守住边界。
...
**自动生成本身就是产品边界的一部分**：手维护多个投影是确定性自动化运营和运维禁止的反模式；任何一处投影若需要"手改"，先回头修 registry / contract，再让生成器输出。
...
§8.1–8.5 是协议规范与产品决策；**运行时与 Registry schema 的具体落地按"真实需求触发"原则推进**。zw-brain 现阶段无外部 Agent 接入排队，按确定性自动化运营和运维「只为真实需求建复杂度」拒绝提前盖楼。
...
确定性自动化运营和运维下，主仓只保留高频核心与底座，长尾能力默认转向外部能力包路径。
...
**单一事实源**：`scripts/preflight.sh` 的 while-read 块（段号 → 脚本路径 → 检查 desc 三列）。本附录**不再复制**该表——任何"doc 抄 script 一份让 reviewer 同步"都是确定性自动化运营和运维反模式（本 PR review 第 1 轮 R-002 finding 即此）。
