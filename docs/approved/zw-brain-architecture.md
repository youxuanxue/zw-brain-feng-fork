...
| 十一、关键设计主张 | 本基线最重要的 R1-R15 设计主张 | 决策审计 |
...
项目代号 `zw-brain`（"政务数据大脑"）继续沿用（代码、文档、客户口径已用），但**产品身份**是"数据共享网关"。
...
旧平台已经证明：菜单多、系统多、接口多，不会自然汇聚成好产品。原始证据（见附录 B 的接口清单、库表与使用日志）进一步表明：旧平台代码面和数据库面极大，但真实调用高度集中在少数能力簇与少数高频链路上。
...
OPC 不是"少人硬扛"，而是"架构、流程、工具都必须服务于一人可运转"。因此：
...
但这些要求的正确落地方式，是把合规与治理**嵌入主旅程**，而不是做成又一套需要用户理解和切换的产品层：
...
> 本章不展开全量数字 Pareto 明细，而只提炼那些会改变架构决策的结论。原始证据见附录 B 的旧平台接口清单、数据库结构与使用日志统计。
...
### 3.2 真实业务价值密度：291 离线 HTML 页面 + 18 项一级目录的高频分布

`old/html/` 下共 **291 个离线 HTML 页面**（18 个一级目录），分布如下：

| 一级目录（业务模块） | 页面数 | 占比 | 新平台中归属 |
|---|---|---|---|
| 运行监控 | **37** | 12.7% | §3.4 第 6 类外部依赖（dsp_monitor 50 表对接集团运维监控平台） |
| 数据直达 | **36** | 12.4% | J1 子旅程 + §3.4 省市间数据通道 |
| 供需对接 | **36** | 12.4% | J1 供需对接子流程（待复议/责任分发细化） |
...
3. **18 项一级目录 → 8 页面是 56% 减法**——Jobs 式收敛正确，收敛逻辑见 §5.2.1 收敛表。

### 3.3 真实强状态领域：6 态 catalog status + 5 步申请 + 5 维度异议

| 领域 | 真实状态机 | 来源 | 处置 |
|---|---|---|---|
| **目录** | `data_catalog.status` 6 态（0 草稿 / 1 待审 / 2 审批通过 / 3 驳回 / 4 发布 / 5 下线）+ 2 应用层计算态（"国家通道转报中"/"撤销中"，由业务逻辑派生，不在 XML enum） | `dsp_catalog.xml:406-408` | §9.2 `Catalog` 标 6 态 + 2 计算态 |
...
| **申请审批** | 5 业务节点（编制 → 校核 → 汇总 → 响应 → 反馈）；XML `data_business.status` 实现为 0-6 共 7 态序列 | `dsp_require.xml:23-25` + 立项会议 1:33-1:42 | §9.2 `Application` 标 5 节点 + ia §J1 供需对接子流程 |
...
| **目录编制双轨** | 政务目录 vs 国家扩展要素目录（不同 status 机 + 不同审批路径，`data_ext_elem_catalog_compile_task` 独立流程） | `dsp_catalog.xml`（`data_catalog` + `data_basic_elem_catalog` + `data_ext_elem_catalog_compile_task`） | §9.2 `CatalogModel` 标双轨 |
| **租户/区划** | 多级区划（省/市/区）+ 跨地市协作 | `dsp_bsp.xml` `pub_org` + `dsp_connect` dc_* | §9.2 `TenantOrg` 标多级 |

**强状态领域结论**：§9.2 10 个 load-bearing 概念被真实状态机事实充实：不再是抽象"概念表"，每条概念都能溯源到具体旧库表 + 真实业务字段。**前台收敛可以激进（B1 后台 / 不做大屏），后台领域不能简化**（6+2 态 / 3 共享态 / 5 节点 / 5 维度全保留）。

### 3.4 外部依赖证明：哪些必须保留为外部底座

旧平台反复依赖的身份、权限、监控、区块链、国家通道、推理平台等能力，说明新平台不应在内部重造。基于 `old/12-datastructure/` 17 模块 678 张真实表事实，零兼容新平台的外部依赖边界必须显式声明，防止 Wave 2/3 复造。
...
| 集团数据治理中心 | 数据清洗/质量/血缘**不是本平台能力**，属数据平台产品部（红军原话"得去联系石云志石总那边的"）；本平台仅通过融合服务系统**注册并代理**治理后接口 |
...
|---|---|---|---|
| **消息中心** | `dsp_message` 独立模块（9 表） | **不复造，shared/notification 极薄封装对接集团统一消息** | 基础设施层，不是业务流程 |
| **集团运维监控平台** | `dsp_monitor` 50 张独立表（服务调用/失败分析/告警工单/巡检/拨测/知识库） | **不复造，对接集团统一运维监控** | 旧平台 37 页运行监控页面密度极高；新平台 B1.1/B1.2 后台仅消费监控数据，不重建运维体系 |
| **集团数据治理中心** | `dsp_pipelines` 50 + `dsp_perform` 16 + `dsp_metaresource` 含 graphdb 5 = 71+ 张（ETL 编排 batch_job/etl_meta + 质量评分 kpi_index + 血缘追踪 graphdb_node/relation） | **不复造，红军原话"得去联系石云志石总那边的"** | 71+ 表是真实规模，远大于"数据存证 10 表" |
| **集团数据安全中心** | `db_meta_database_permission` 等脱敏隐私表分散 | **不复造，仅 B1.2 后台提供策略入口** | 数据分类分级 / 敏感识别 / 脱敏 / 密钥管理由集团数据安全中心承担 |
| **数据存证 / 数享链** | 主体 `dsp_block` 10 张 + 跨地市 `dsp_connect` 57 张 dc_* 系列含存证 | **能力包外部化（adapter）；不在主仓 domain；外链 down 不阻塞业务** | 存证不止 dsp_block，跨地市 connect 也含存证语义 |
...
立项会议红军强调"数据不一定来自基层"；一表通从默认路径**降级为可选预填 adapter**；`ROLE_ORGAN_OPERATER` 旅程改为"接到补差任务才出现"。详见 [docs/approved/research-yibiaotong.md](docs/approved/research-yibiaotong.md)。

### 3.5 真相导出的架构结论

从 §3.1（17 模块 678 表）+ §3.2（291 离线 HTML + 18 项一级目录）+ §3.3（6+2/3/5/5 真实状态机）+ §3.4（6 类外部依赖）的真相事实出发，落下**七个**核心结论：

| # | 结论 | 真相来源 |
|---|---|---|
| 1 | **内建只承接 J1 + J2 两条核心旅程，B1 后台仅作合规底线** | §3.2 页面密度 + §3.1 dsp_require 5 步流程主线；红军立项原话"用得最多还是核心 J1+J2" |
| 2 | **强状态领域全保留，前台收敛激进，后台领域不简化** | §3.3 6+2 态 catalog / 5 节点申请 / 5 维度异议 / 3 共享态 / 3 物化形式 / 双轨编制 / 多级区划 — 这些是 7 条不可减语义 |
| 3 | **长尾能力默认外部化**（§3.4 6 类外部依赖 + §5.6 13 项不进 IA） | §3.1 dsp_monitor 50 表 / dsp_pipelines 50 表 / dsp_basesubject 81 表 — 这些是真实运行但不该进主仓的能力 |
| 4 | **控制面保持最小可用**（Capability Registry / Policy / Audit / Compatibility / Rollback） | 旧 dsp_bsp 76 张表 + dsp_service 27 张表的控制面是过度膨胀的反例 |
| 5 | **客户差异通过 R14 三引擎 + 多租户策略 + 外部能力包**承接，**不通过 fork 后端** | 立项会议红军原话"各项目门户、表单、流程都个性化；旧平台靠改代码 + 改数据库" |
| 6 | **旧平台前端的多 SPA 拼接是反模式**——zw-brain 必须单一 WebUI + 同一能力契约 | §3.1 旧 7 个独立 SPA → zw-brain 单一 zw-brain-web/，避免 portal-vue + app-center-web 等割裂导致的角色困惑 |
| 7 | **旧平台 18 项一级目录 → 56% 减法是 Jobs 式正确**，但收敛逻辑必须明示防回潮 | §3.2 HTML 密度 + §5.2.1 收敛表 |
...
4. **扩展路径 AI 原生**：新增功能默认在外部 Agent 中构建（ANP 平台 / Cursor / 其他工具皆可），通过 AgentRuntime 声明式协议（`AGENT.yaml`）注册进入平台，而不是继续在主仓库里长菜单、长模块、长分叉（详见 §八 / R15）。
...
平台内部真正稳定的最小单位不是页面、API、脚本或 Agent，而是 **Capability**。
...
| Long-tail | 项目特有、低频、试验性、行业特化 | 外部 Agent 经 AgentRuntime AGENT.yaml 注册（详见 §八 / R15） |
...
Registry / Policy / Audit / Compatibility / Rollback 这些控制面能力是必须的，但它们的目标只有一个：**把主旅程做稳，把长尾扩展纳入秩序**。
...
| **B1 看全局→处异常**（后台支撑面，非旅程） | 让全局可用、异常可追、合规可证 | `ROLE_BUSIAUDIT` + `ROLE_SECURITY_AUDIT` + `ROLE_SECURITY_ADMIN` + `ROLE_SYSTEM` | 合规运营 + 平台接入 | **后台支撑面**：少数管理员/审计员使用；不进普通用户主导航；不是产品差异化重点；存在的意义是合规底线 |
...
- **编号空间区分**：`J1/J2` = 用户核心旅程；`B1` = 后台支撑面（非旅程）；`R1-R15` = 架构约束设计主张（§11）；`D-编号` = GATE 后决策追加

### 5.2 WebUI 核心页面

产品界面收敛为 **8 个主页面**，继续服从 `≤<!-- stat:zwbrain.webui-pages-cap -->8<!-- /stat -->` 的硬上限。普通用户主心智应聚焦核心旅程页面（P1-P5/P7），后台支撑面（B1.1 / B1.2）仅管理员访问。
...
| P7 共享专区 / 专题包 | J1 找数→用数 + J2 挂数→维数 | 主题化聚合与订阅入口；旧平台"几乎不用"是入口埋深 + 无订阅，本基线作为 K11 必保留升级 |
| B1.1 合规与运营 | B1 后台支撑面 | 审计回放、统计、异常、追责；仅管理员/审计员，不进普通用户主导航 |
...
| 资源购物车（res-purchase-car） | portal-vue `/res-purchase-car` | （并入资源管理） | **待业务方 sign-off**（资源批量申请暂存是否补入 P3） | ⏳ pending |
| 反馈入口（feedback） | portal-vue `/feedback` | （并入工作台） | **待业务方 sign-off**（与异议区分的独立产品体验反馈） | ⏳ pending |
...
**Jobs 式判断**：旧平台 18 项一级目录 + 7 个独立 SPA → zw-brain 8 页面 + 单一 WebUI，**真实减法 56%**。这不是激进，是回到产品本质——客户日常用的就是 J1+J2 这两条链。
...
- **B1.1 合规与运营**：AI 做"调查摘要助手"——证据分类、事件归纳、工单/知识建议串联；不覆盖原始审计证据，也不替代人工调查结论。
- **B1.2 平台接入后台**：AI 做"审核缺项助手"——标缺项、草拟退回意见、提示确认边界问题；不自动批准、自动发布。
...
| MCP | IDE / Claude / Cursor 类 Agent（外部 Agent 经 AgentRuntime 声明式协议 `AGENT.yaml` 暴露，详见 §八 / R15） | 中 |
| A2A | 外部 Agent 平台（经 AgentRuntime 声明式协议暴露与调用，详见 §八 / R15） | 中 |
...
文档、OpenAPI、CLI spec、MCP manifest、A2A card 都应由同一 registry / contract 派生，而不是人工分别维护。**自动生成本身就是产品边界的一部分**。
...
L4 Runtime & Adapters
  Built-in Skills | Registered Packages | Adapter Skills | Tool Bindings | AgentRuntime（声明式 Agent 执行内核，anp-agent/v1.2，详见 §八 / R15）
...
| 运行形态 | Phase 1 默认 Embedded SDK；Standalone HTTP 留 Wave 3+ 评估 | 与 R4「控制面纤薄」一致；触发切换的条件 = 多租户隔离压力 / 跨集群部署 |
...
基于 `old/12-datastructure/` 真实表事实补充状态机 + 概念到真实表簇的完整溯源（让每个概念都能追踪到旧库具体表）：
...
| `Catalog` | 目录节点与主题分类；`status` 真实 6 态（0 草稿/1 待审/2 通过/3 驳回/4 发布/5 下线）+ 2 应用层计算态（"国家通道转报中"/"撤销中"，由业务逻辑派生）；`shared_type` 真实 3 态（1 无条件 / 2 有条件 / 3 不予共享） | `dsp_catalog.xml`: `data_catalog` (51 字段), `data_catalog_item`, `data_cascade_catalog_topic` |
...
| `CapabilityPackage` | 注册能力包元信息；**zw-brain AI 原生重构引入的承重抽象**，无旧库继承——为 AI 原生扩展模式新发明 | **无旧库表对应**——是 R7 / R14 / R15 引入的新概念；外部 Agent 经 AgentRuntime AGENT.yaml 注册路径 |
...
- **alembic 不进入产品基线**：对全新项目维护迁移链是历史兼容思维的副产品，干扰主目标 J1 黄金链路聚焦
- **alembic 启用条件**：第一个真实客户上线 + 第一次生产 schema 变更时，把当时的 schema 作为新 baseline 启动 alembic（那时才有"迁移"的真实需求）
...
> **旅程编号说明**：本节按 J1 找数→用数 / J2 挂数→维数 / B1 看全局→处异常（后台支撑面） 命名。
...
- 不维护数据库迁移链（schema 用 SQLAlchemy `drop_all` + `create_all`，详见 §9.6）
...
影响：客户差异只能通过配置、多租户策略、外部能力包实现，不能回到客户分叉后端模式。客户差异中**项目级流程 + 表单 + 推荐规则**由 R14 三大引擎承接（不进主仓代码）。
...
影响：7 角色码（`ROLE_SYSTEM` / `ROLE_BUSIAUDIT` / `ROLE_ORGAN_MANAGER` / `ROLE_ORGAN_OPERATER` / `ROLE_SECURITY_ADMIN` / `ROLE_SECURITY_AUDIT` + `tag_lead_dept` 标签）冻结；新增需走 GATE + 业务方 sign-off；由 `policy.assert_no_legacy_role_codes()` + preflight 段双层兜底。
...
影响：任何"角色定义 / 业务流程 / 状态机"类决策必须有业务方（客户产研代表 / 海若产品部业务方）sign-off 才能进 D-编号；立项会议输出（白皮书 + 转写录音 + 角色梳理）是 GATE 决策的强制输入，不是参考材料。
...
影响：外部 Agent 不论由 ANP 平台、Cursor 还是其他工具构造，进入 zw-brain 必须以 AgentRuntime 声明式协议（`AGENT.yaml`）形态声明并通过 AgentRuntime 执行内核运行。zw-brain 侧产品决策（信任等级默认、运行形态、鉴权 / 多租户 / context 选型）见 §8.2 决策表；协议规范单一事实来源 = `docs/agent-runtime/*`，AgentRuntime 升级不触发本基线同步。与 R7（长尾外部化）/ R14（项目级可配置）联动：外部 Agent 仅承担长尾能力与配置草稿生成，禁止承接 §8.5 禁止外部化清单。
...
- `old/html/`（18 个一级目录，291 个离线 HTML 页面）：实测页面密度详见 §3.2 表
...
| 工程术语不得出现在前端 UI（R12） | 待接入 | 建议新增 preflight 段做 `zw-brain-web/` grep 黑名单：`package` / `projection` / `capability` / `write-with-audit` 等 |
| 角色 / 业务流程 / 状态机决策必须业务方 sign-off（R13） | 软约束 | 建议同步进 `dev-rules/global/CLAUDE.md` §2；preflight 暂不强制（涉及人工审批，硬化收益低） |
...
- 后续修订请在本文上原位演进，新增决策追加 D-编号（见 CLAUDE.md §决策记录），新增架构主张追加 R-编号。
