...
> **v4.1 R15 通告（2026-05-20）**：本文表格中提及的 `alembic/versions/0005_*` / `0008_*` / `0009_*` 已在 v4.1 二轮再砍中**一并删除**（alembic 整体退役，R15）。新项目用 SQLAlchemy `Base.metadata.create_all()` 直接建表，不再走迁移链。文本保留作为旧 BSP → zw-brain handoff 心智参考。
