...
### 1.2 确定性自动化运营和运维：单一事实源，不为 catalog / metadata 开第二套治理中心
...
本节是 dsp-catalog3 / dsp-metadata3 相关数据模型与迁移映射的单一事实源；`docs/approved/zw-brain-data-model.md` 只保留 canonical model 通用结构，并引用本文。
