...
3. 能强化 J1 找数→用数 / J2 挂数→维数 主旅程闭环，或支撑 P5 供给侧治理、B1.1 合规与运营、P7 共享专区 / 专题包。
...
1. 把门户、菜单、按钮权限、监控、消息、调度等投影 / 底座能力当成业务事实源。
...
- 不把 projection 反向写成事实源；B1.1、P7 读模型均只能消费 canonical、adapter receipt 和 audit evidence。
...
| `data-operation-board-front` | 前端 / - | B1.1 视觉输入 | P2 | 只服务运营看板 projection，不成为主事实源。 |
| `dataresource` | 后端 / 25 | 默认不重构，必要时作为专题包内容源 | P3 | 旧数据资源库是招标补齐型建库向导，approved 已倾向不默认恢复。 |
| `datasecurity-front` | 前端 / - | 不重构 | P3 | 安全前端不进入主产品核心。 |
| `datasecurity-service` | 后端 / 225 | B1.1 风险监测 / 外部安全 adapter | P2 | 去重分类分级 / 脱敏 / 加密引擎，只保留风险摘要、处置证据和审计联动。 |
...
| `dsp-monitor` | 后端 / 587 | 外部观测底座 / B1.1 投影 | P2 | 监控告警不是主旅程聚合，不能反向驱动业务状态。 |
...
| `metricsmgr-front` | 前端 / - | B1.1 视觉输入 | P2 | 不迁前端代码，只参考指标展示诉求。 |
| `metricsmgr-service` | 后端 / 99 | 指标 projection / B1.1 adapter | P2 | 未封版，不能把直连业务库的指标平台迁成事实源。 |
...
1. `dsp-objection-handling-reconstruction-plan-v1.md`：已产出，补齐 J1 / J2 异议子流程强状态闭环。
...
5. `compliance-ops-adapters-reconstruction-plan-v1.md`：已产出，处理安全、标准、指标、监控、督导等 B1.1 外部 adapter。
