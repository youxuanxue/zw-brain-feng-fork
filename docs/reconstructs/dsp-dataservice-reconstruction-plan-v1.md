...
这符合 Jobs/OPC：先替代最高频真实价值链路，同时不给长尾后台页面永久席位。
...
`old/12-datastructure` 补强的是旧库表级事实，不改变 Jobs/OPC 迁移方向。它证明旧 `dsp-dataservice` 的承重对象确实集中在服务本体、服务分类、代理通道、授权策略、调用统计、运行监控和上链日志：
