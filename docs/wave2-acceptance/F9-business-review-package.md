...
driven_by:
  ...
  - docs/wave2-acceptance/F9-prep-checklist.md
  - CLAUDE.md D34
---
...
> - `docs/wave2-acceptance/F9-prep-checklist.md` §七
...
>    评论 business-signoff / plan.yaml L227+L234 status=completed）
...
|---|---|---|---|
| 医疗救助信息（seed URN `370000307013000000/000045`；旧平台真实 code `003601020201001`） | seed 真实 catalog_entry（真实 dump 命中 ✅） | **必含** — 核心、已在 seed、demo 命中 | ☐ 必含 / ☐ 排除 |
...
见 §5 — 统一 PR label `business-signoff: e3.F9`，实质内容（P0/P1/砍 + 每个专区的
...
| 项 | 形态 | 真值源章节 |
|---|---|---|
| P7 共享专区主入口 + 专题包列表 / 详情页 | 1 个 Vue 页面 | plan v1 §四 L217 + F9-prep §五 |
| TopicPackage 投影表 | 物理表（Base.metadata.create_all，不进 alembic） | plan v1 §3.1 |
| capability slug | Registry manifest（清单见 plan v1 §四） | plan v1 §四 + F9-prep §三 |
| 5 消费面（webui / api / cli / mcp / a2a） | 投影，统一 contract 生成 | plan v1 §四 L217 |
| sd-default 山东标杆 fixture | tests/fixtures/topic_package/*.json | F9-prep §四 |
| e2e 测试 | J1 找数 + J2 运营 + D7 守护 | F9-prep §六 |
...
业务方对 §1 / §2 / §3 / §4 全部 sign-off 后，**统一以单一 PR label
`business-signoff: e3.F9` 触发**（与 `scripts/promote_signoff.py` grammar 一致，
scope=`eN.FX` 模式，与 F8 PR #151 / E2 PR #143 / E4 PR #147 / E1 PR #143 同模式）。

label 是开关；实质内容（4 项决策结果）写进 **PR 评论 body**，按 §5.2 模板。
...
- [ ] **A**：`.twin/e3-wave2-engines/plan.yaml` F9.actual_evidence 末段追加
      `[SIGNOFF-CLOSED <日期>] covers F9-launch | by <角色> | vehicle PR #162 评论 business-signoff | priorities=... | offline_pending=... | matrix=... | boundary=... → 启动 F9`
      （格式与 plan.yaml L227 F8 同模式）
- [ ] **B**：PR #162 加 label `business-signoff: e3.F9`（自动触发 `promote_signoff.py`
      升级对应 .feature Status）
- [ ] **C**：CLAUDE.md 追加 `D<编号>` 决策条（编号 = sign-off 当天最大 D + 1，
      不预设；当前最大 D33.d，故下个为 D34，但若本次 review 与他 D 并发以实际为准）
- [ ] **D**：新 worktree 启动 F9 worker（按 F9-prep-checklist 八段走）

**机械守卫**（确定性自动化运营和运维）：

- A 项落盘后由 `preflight` 段 26 `twin-workspaces` schema 校验 + 段 34
  `wave-snapshot-sync` 反向锚点解析自动 verify
- B 项由 `scripts/promote_signoff.py` PR label 触发器消费（已存在）
- A+C 一致性由 **段 54 `check_signoff_landed.py`**（D35）机械校验：本文 `status: approved`
  时，`.twin/**/plan.yaml` 必须有匹配 `[SIGNOFF-CLOSED ... e3.F9]` evidence + `CLAUDE.md`
  必须有提及 `e3.F9` 的 D-编号；缺则 FAIL。**「C/D 靠人记忆」债已由 D35 关闭**（不再 debt 化）
