...
这是 zw-brain 的开箱切换体验，不是一个长期角色。客户现场要从旧平台一键导出真实数据，迁移到 zw-brain，并在验收时证明旧目录、资源、元数据、申请、审批、授权、质量、血缘都被承接；M0 是唯一处理迁移执行、批量导入、对象映射完整性、重跑、回滚和验收移交的页面。迁移完成后，日常工作交给 3 旅程（J1 找数→用数 / J2 挂数→维数 / J3 看全局→处异常）下的 7 角色（详见 `docs/approved/zw-brain-roles-v2.md`），不保留运行时双轨。
...
12. 验收通过后关闭迁移模式，把后续资源维护（J2）、目录发布（J2 + `ROLE_BUSIAUDIT`）、审批授权（J1）和合规督查（J3）移交给对应 7 角色（详见 `docs/approved/zw-brain-roles-v2.md`）。
