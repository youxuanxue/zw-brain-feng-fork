...
| `trusted_gateway` | 网关注入 principal、签名和时间戳头。 |
...
| `callbacks:deliver` | 标准 callback delivery scope。当前 `/callbacks/{wait_id}` 使用签名验证，不直接依赖该 scope。 |
| `admin:runtime` | 运维接口和跨 owner 访问。 |
...
| `timeout` / `tool_error` / `sandbox_error` / `model_error` / `internal_error` | 500 | 执行、工具、沙盒、模型或内部错误。 | 记录 `trace_id`，读取 task error 和日志。 |
...
`/state` 返回 session、task 投影和 message 投影。`tasks[]` 会额外包含：

- `resumable`：当前 task 是否可调用 `/resume`，由 `status == waiting_input` 决定。
- `resume_after_restart`：等待态是否承诺跨服务重启恢复，由 `status == waiting_input`、`persistence == persisted` 和 Runtime checkpoint 能力共同决定。
- `run_kind`：Task 来源，例如 `session_run`、`scheduled_run` 或 `manual_shortcut`。
- `persistence`：Task 持久化策略，例如 `none`、`session` 或 `persisted`。

Message 只存会话级最终投影，流式 `message_delta` 原始增量默认不作为最终消息保存。页面刷新时优先用 `/messages` 恢复对话历史。
...
| `mode` | <code>one_shot &#124; durable</code> | 否 | `one_shot` | 兼容字段。等待态是否可恢复由当前状态决定；新接入方优先使用 `persistence`。 |
| `run_kind` | <code>session_run &#124; scheduled_run &#124; manual_shortcut</code> | 否 | `session_run` | Task 来源/触发类型。 |
| `persistence` | <code>none &#124; session &#124; persisted &#124; null</code> | 否 | `null` | 持久化策略；未传时 `mode=durable` 映射为 `persisted`，其他情况映射为 `session`。 |
...
| `subject_user_id` | <code>string &#124; null</code> | 否 | `null` | 代表哪个用户执行；不同于当前 principal 时需要 `tasks:start:any_user`。 |
...
| `agent_manifest_snapshot` | Task 启动时冻结的 Agent manifest。 |
| `metadata` | 业务透传元数据。 |
| `effective_trust` | 本次执行实际 trust level。 |
| `workspace_id` | 本次执行 workspace。 |
| `mode` | `one_shot` 或 `durable`，兼容字段。 |
| `run_kind` | Task 来源/触发类型。 |
| `persistence` | Task 持久化策略。 |
| `status` | `pending`、`running`、`waiting_input`、`completed`、`failed`、`cancelled`。 |
| `final_output` | 最终输出，任务完成后可读。 |
| `error` | 失败信息。 |
| `wait_type` / `interrupt_payload` | 等待态类型和提示信息。 |
...
| `middleware_snapshot` | 本次执行的 middleware / policy / limits 快照。 |
...
普通 principal 只能读取自己拥有的 task；`admin:runtime` 可跨 owner 查询。
...
| `system_prompt_append` | <code>string &#124; null</code> | 否 | `null` | 恢复时追加系统提示。 |
| `system_prompt_override` | <code>string &#124; null</code> | 否 | `null` | 恢复时覆盖系统提示。 |
| `dynamic_context` | <code>object &#124; null</code> | 否 | `null` | 恢复时动态上下文。 |
| `actor` | <code>object &#124; null</code> | 否 | `null` | 实际操作人信息。 |
| `idempotency_key` | <code>string &#124; null</code> | 否 | `null` | 恢复请求幂等键。 |
| `metadata` | `object` | 否 | `{}` | 业务透传元数据。 |

调用方不能直接指定内部 `resume_type`。Runtime 会根据 task 当前 `wait_type` 归一化为内部 `ResumeEnvelope`。
...
响应包含：`task_id`、`items`、`limit`、`after_sequence`、`last_sequence`。
...
- `message_delta` 和 `model_thinking` 默认只持久化 metadata，不保留原始 delta 文本。
- `task_completed.payload.final_output` 默认持久化为摘要字段 `final_output_summary`，完整结果以 `TaskRecord.final_output` 和 message projection 为准。
- 客户端应使用 `(task_id, sequence)` 去重，不应使用 `event_id` 排序。
...
| `PUT` | `/workspaces/{workspace_id}/files/content?path=<file>` | `workspaces:write` | 写入文件。 |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `GET` | `/agents` | 返回可展示的 Agent summary 列表。summary 字段为 `agent_id`、`name`、`version`、`description`。 |
| `GET` | `/agents/{agent_id}` | 返回对应 Agent manifest 原始结构。 |
| `GET` | `/agents/{agent_id}/files/content?virtual_path=<path>&download=false` | 读取 Agent 声明 resource mount 下的文件。 |

Agent API 需要 `agents:read` scope。Runtime 会按 discovery policy 过滤可见 Agent；读取资源文件时还会要求目标 Agent 对当前 principal 可见并允许 chat/A2A 发现。
...
- `TaskRecord.agent_manifest_snapshot._runtime.context_package.checksum` 用于确认本次模型请求使用的 ContextPackage 投影。
- `TaskRecord.agent_manifest_snapshot._runtime.runtime_context_snapshot` 保存安全 token stats、context utilization、trim decision kinds；不含 selected message ids 或正文。
...
| `GET` | `/runtime/ready` | 就绪检查。ready=false 时返回 503 和 preflight report。 |
...
`/runtime/diagnostics` 包含：deployment snapshot、preflight ready/ok、deployment_profile、runtime_core、数据库迁移状态、checks 和 capability_checks。Task snapshot 中的 `_runtime.resolved_model_profile` 与 `_runtime.runtime_resolution_explanations` 可用于 operator 侧追踪模型解析结果，以及 Runtime 如何将 Agent capability request 解析为 effective grants。
...
| `run_data_retention_maintenance(...)` | 执行数据保留清理。 |

## 14. 环境变量配置

`runtime_settings_from_environment()` 的优先级为：显式构造参数 > 环境变量 > 默认值。
...
| `AGENT_RUNTIME_SESSION_STATE_ISOLATION` | `session_state_isolation` | `shared_workspace` | session 状态隔离策略。 |
...
| DeepAgents core 启动失败 | `runtime_core=deepagents` 但未配置 `database_url`。 | 配置 Postgres，或测试环境改用 `runtime_core=fake`。 |
