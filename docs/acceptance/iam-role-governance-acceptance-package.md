...
> 身份治理产品页（媲美旧平台 BSP 运营能力面）。2026-06-25 起，`旧权限映射审核`
> 不再作为平台运维员 WebUI 模块出现，策略候选能力仅保留非 WebUI 契约面。本验收每点挂
...
- **运营能力面**：身份治理 2-tab（用户与角色 分派/撤销/停用 + 谁能访问什么只读矩阵）+ 5 能力（全 ROLE_SYSTEM 门控 + 全程审计）。
...
| 8 | 身份治理页不再出现旧权限映射审核模块，避免把迁移期审核误当日常身份治理入口 | 通过 | `身份治理 actor 管理面 e2e（b12_iam_governance + customer_acceptance + twin_browser）` |
| 9 | 5 消费面（WebUI/REST/CLI/MCP/A2A）契约一致；策略候选能力已从 WebUI compatibility 退出 | 通过 | `5 消费面契约一致` |
