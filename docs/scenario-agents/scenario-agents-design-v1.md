...
> 落地形态：每个场景智能体 = `agents/<id>/AGENT.yaml` + `capabilities.json`（sidecar），由平台**运维员**在 B1.2 管理面构建/配置/启停。
>
> **两个被反复混淆的层级，先钉死：**
...
| 承载 | `AGENT.yaml(tools:[])` + `capabilities.json` sidecar 的 `capability_tools[]` 经 `ZwBrainCapabilityProvider` 注入既有 `skill_id` | `AGENT.yaml` 的 `tools[]` 写 `kind:api`/`kind:a2a`（受 schema 校验），或作为外部应用经 5 消费面之一（API/A2A）消费 |
...
5. **注入机制的承载来源**：`capability_tools`/`skill_id` **不经 `agent.schema.json` 校验**（schema 里这两个字段出现 0 次、根 `additionalProperties:false`），由 `capabilities.json` sidecar + `ZwBrainCapabilityProvider` 承载；schema 只约束 `AGENT.yaml` 本体（其 `tools[]` 反而是用来声明 api/a2a/runtime/mcp/skill 工具的）。写 sidecar 的人要知道 `capability_tools` 错了不会被 schema 拦。
6. **B 类外部接入＝"规范就位、运行时触发式未实装"**（§8.6 + t1-readiness §1）：**T1（首个真实外部 Agent 接入需求）未 fire** 前，Registry 4 字段、`validate`/`doctor` 真实校验、preflight 段30 **均不 land**，原 spike 工具链已随 **D46.e 退役须重建**，原估 1 天已失效须重估。当前 A2A 是 AgentRuntime 内核**本地 agent 间调用**，**不是给外部系统直连 zw-brain Capability 的公网 API**。→ B 类任何"外部独立 agent 即插即用"的措辞都是假面，须改为"触发 T1 后方可"。
...
- **C6 运行时就绪度（新增）**：本期能否真交付，取决于承载面是否已实装。A 类 sidecar 注入端到端已跑通＝就绪；B 类外部接入面 T1 未 fire＝未就绪，只能立规范 + 试点。**就绪度不准则"可签收"判定不可靠**——这是 B 类不进本期 Wave 的根因。
...
`exposes_chat:false`（页内嵌副驾）· 形态：意图解析 + 可行性评分 + TOP-N 推荐 + 术语对齐。**它跑通即证明 `AGENT.yaml + capabilities.json sidecar 注入` 这套承载端到端可交付——是后续所有 A 类乃至 B 类共用底座的地基。**
...
| onestop-guide-copilot 一件事一次办导办 | 民生 | embedded | `data_item`/事项 FLOW + 材料共享 |
...
- **若只能做一个**：**A①**。它已 live、风险最低、J1 首步最高频、升级现存 agent（sidecar 注入已端到端跑通），签收即一次性证明承载形态可交付——胜过同时铺 20 个半成品假面。
...
> 附：所有 `skill_id` 已对 `docs/agent_integration.md` 校验；落地写 `capabilities.json` 时再逐一复核（`capability_tools` 不被 schema 拦，错了要靠 review 抓）。
