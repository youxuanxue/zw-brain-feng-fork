...
- D48 [06-03] **data-model-referential-integrity**（架构门，产品研发负责人 sign-off）：批准硬约束「zw-brain 永不留孤儿行——删父级联或拒绝，由系统机制强制」。17 边 FK+ON DELETE CASCADE（A12+B5复合，干净库 drop&recreate 即带）；C类7边经真库实证父写子时不保证存在 + 码列多态 → honest 降级、由 preflight **段67** check_orphan_rows 全边孤儿守卫兜底；缺陷4 悬挂 ref=数据层只读派生就绪信号（仅 topic.package 详情序列化路径计算、当前无 webui/消费面调用该路径→用户侧不可见 by design，用户可见悬挂呈现+可点目录详情随 J1；2026-06-03 负责人走查实证、非用户侧可见功能）；段67 折入 negative-and-guardrails（护栏真相面不留例外）。缺陷2(D47.a)/3 记债暂停、上游真因(subscribe_job/码列多态 D47.b)另立项。证据 evidence.json；全文 docs/decisions/data-model-referential-integrity-design.md
