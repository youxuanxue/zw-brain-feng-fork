...
- D46 [05-30] **feature-status-as-function**（架构门，pending 产品研发负责人 sign-off）：status 不存储，由 SPEC(.feature)+MEASUREMENT(.testing/status/measurement)+SIGN-OFF(.testing/signoff/ 账本 covers) 现算（feature_status_lib.py）；删 .feature 全部 # Status（Backlog→# Deferred）+ 删段58对账守卫 + promote_signoff；加段60/61/62（测量校验/生成--check/禁手写）；Done=绿∧签字（计数不在此存储，实时现算见 .testing/status/feature-status.md）；plan.yaml status 留作 supervisor 执行态不入函数（D46.a）；SIGN-OFF 独立账本 .testing/signoff/ 修签字寄生 twin 破洞（来源无关 + 修误标 supply-demand/漏签 trust-level）+ 段63 账本守卫 + 段54 收敛为账本单源（D46.b）；SPEC 是进飞轮唯一入口、无 .feature 不进状态、立规则不加守卫（D46.c）；PR 合并自动落账（D46.d：label signoff:<scope> + body 机读块 → signoff-ledger.yml 调 signoff_from_pr.py 取 approvers/merged_at/PR# 生成账本提交回 main，GitHub 是录入口账本是真相，单测兜底）；.twin/ 整体退役（D46.e：执行计划不再承载真相 → git rm -r .twin + 补 e5 3 webui feature + 段38 三角收敛 SPEC↔test 双边 + 删 # Twin-F + 删段26 check_twin_workspaces；不留历史兼容）；测量轴 test-runner 无关化（D46.f：green() 纳入 e2e — test_refs 认 tests/**.spec.ts + capture --with-e2e 实跑 Playwright 回填同一产物，修 webui 因 pytest 轴盲永卡 Ready；全 38 non-Done feature 逐个代码核实校正 workflow 过度悲观，唯一过度声称 a2a-hardening（挂名不测）→ Backlog，其余 InTest 对本期核心诚实、业务深度 by-design 延后；干净 seed 库实跑 webui×3：action-role-binding+routing-cleanup→Done，pages-real-data 因 seed 数据脆留 Ready 不冒绿；计数不在此存储、实时现算见 .testing/status/feature-status.md；全文 docs/decisions/test-sufficiency-convergence.md 第二轮）；**测量轴信任锚 git_sha→内容指纹（D46.g）：squash-merge 把分支测量孤儿化致段60 永久 WARN + green() 对陈旧产物可误标 Done（实证误报 request_service DB 回源"未做"）→ 改 feature_fingerprint=sha256(.feature+引用测试文件) 作锚，capture 写指纹、green() 要指纹匹配、段60「绿但指纹陈旧→FAIL」（squash 免疫、安全 FAIL 不炸 main）；CI preflight 做便宜指纹检查不需 seed、重采本地做；D11 真数据进 CI 降为独立轨（只抓真数据回归，不再是状态诚实前置）；ops 待办=开 main 分支保护 require-CI-green（✅ 2026-06-01 已开：test (py3.12)+lint (ruff)+legacy-import-smoke 必需，A-lite；配置真值源 docs/ops/main-branch-protection.md）；#3 顺带把 feature/e2e/j1-credential-revoke 三条 external debt 转 grep 现算、j1-api-call-monitoring 无干净锚保持 external）**；全文 docs/decisions/feature-status-as-function-architecture.md
- D47 [06-02] **c1-demo-removal-credential-honesty**（C-1 余下，承接 #191/D46 后；产品研发负责人 sign-off）：**删演示单** = `seed_snapshot.json` 清空全部捏造演示业务记录（requests/approvals/delivery_tasks/disputes/audit_events/alerts/tickets/knowledge_articles/discovery.resources + provider/capability_packages 演示项），一切围绕真实导入；读路径单一事实源（#191）下空库→诚实空、真库→真实全量；`demo_state_sync.sync_demo_state_views` 硬编码演示 cascade 退役为 no-op（通用待办/查找助手保留）；段 36 收紧 `ALLOWED_FILES=set()` 零 allow-list（全仓零 demo id 字面）。**凭据诚实化** = 真实授权表 `data_apply_authrization` 无 per-grant 凭据（真凭据在网关域 `dsp_service.api_service_app.SECRET`、与 apply_id 无绑定供数），legacy 导入 granted 分支停 `derive_demo_credential` 捏造 AK-DEMO、改写 `credential=None`+`credential_status=not_issued`，`credential.query`/P4 诚实显未签发；段 66 重定向为「granted 分支显式处理凭据态 + 禁回潮捏造」；新建在产单 approve 自动签发（平台自签）保留、与历史未签发并存。业务待确认项（J1 凭据取网关 SECRET 口径 + `apply_id↔service↔app` 绑定供数 = 上游缺供）记债不在本期（D47.a）；历史导入单在线动作混合门控（能解析运行时实体才可动作）承接 `j1-legacy-record-actionability` 债（D47.b）。效果验收通过（D47.c，证据 `.testing/acceptance/c1-demo-removal-credential-honesty/evidence.json`，产品研发负责人本地全栈 real-only 真库走查：granted→未签发 / 新单→已签发 curl / J1+J2+异议主链路 / 权限可见性）；全文 docs/acceptance/c1-demo-removal-credential-honesty-acceptance-package.md + docs/preflight-debt.md 2026-06-02 C-1 条
- D48 [06-03] **data-model-referential-integrity**（架构门，产品研发负责人 sign-off）：批准硬约束「zw-brain 永不留孤儿行——删父级联或拒绝，由系统机制强制」。17 边 FK+ON DELETE CASCADE（A12+B5复合，干净库 drop&recreate 即带）；C类9边经真库实证父写子时不保证存在 + 码列多态 → honest 降级、由 preflight **段67** check_orphan_rows 枚举登记边孤儿守卫兜底；**[06-03 amendment]** 守卫覆盖=常量登记边集合（单一事实源=*_EDGES，非 schema 全反射），原文"全边"措辞收敛为"枚举登记边"——补 catalog_item/delivery_subscription.resource_code 两条原裸奔软引用边（干净 seed 0 孤儿实证）；objection_case.related_application_id 系无 live deref 的松散可选标注（legacy 恒 NULL、无消费方 join），不构成参照边、刻意不纳入（纳入即虚构约束）；缺陷4 悬挂 ref=数据层只读派生就绪信号（仅 topic.package 详情序列化路径计算、当前无 webui/消费面调用该路径→用户侧不可见 by design，用户可见悬挂呈现+可点目录详情随 J1；2026-06-03 负责人走查实证、非用户侧可见功能）；段67 折入 negative-and-guardrails（护栏真相面不留例外）。缺陷2(D47.a)/3 记债暂停、上游真因(subscribe_job/码列多态 D47.b)另立项。证据 evidence.json；全文 docs/decisions/data-model-referential-integrity-design.md
- D49 [06-03] **wave-residuals-approval-depth**（流程/状态机变更，产品研发负责人 sign-off；#200 已签**但漏记 D-编号**，本条补登 D28 GATE 索引、不新增决策也不新签）：**committed 自定义 live approval schema 现真驱动 J1**——`approval_flow_walker.py` 把 live schema(nodes+branches+selection_rules) 走查成有序 ApprovalStep；`request.py` 优先 `find_live_for_scope`(项目级)→schema 驱动，否则原样回落 baseline（baseline 零改动、golden 钉死）。本期 **deliberately 串行**：`always`→串行多级 / `on_decision`→happy-path（approve→下一步、reject→终止由决策时状态机承载，与 baseline 同构）/ `expression`→fail-closed 走 always+warning（**不做 {field,op,value} 条件求值器**：鞍山+全场景纯串行零条件路由，求值器属镀金、待真有按金额分流需求再立项）；不可达/环/无 approval 节点→fail-closed 回落 baseline。**签字已在** `.testing/signoff/wave-residuals-approval-depth.signoff.yaml`（薛娇/产品研发负责人 2026-06-03，账本注明属 D28 状态机变更），本条仅补 D-编号索引、引用既有签字不另起新账本。证据 PR #200 / `.testing/acceptance/wave-residuals-approval-depth/evidence.json`（本地真栈 4/4）+ `test_approval_flow_schema_drives_j1.py` 8 测绿；全文见 `engine-approval-flow.feature`(Done) Landing-Note。
