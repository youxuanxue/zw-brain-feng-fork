...
- D63 [06-19] **iam-pub-user-role-materialization**（架构+角色映射门，D28 GATE；产品研发负责人 2026-06-19 sign-off，decision_only）：修存量用户角色错配——旧导入器只读 pub_user_organ_role（136 行/多 ROLE_WORKER）喂映射，pub_user_role（旧平台真实角色事实表 4187 行）只 stats.bump 不落 binding（role-mapping-manifest 本照其词汇所建＝清单知真相、代码读错表）；生产用户级实测 82/137（60%）有角色用户被判错（gaodaliang 应 BUSIAUDIT+MANAGER+OPERATER+SYSTEM、旧只给 OPERATER）。裁决：pub_user_role 并入产品角色主源（与 organ 合并去重、经映射落 5 角色、受同一 iaf_sub+active fail-closed 门），落主机构 pub_user.ORG_CODE，join 按 pub_user.ID 避 USER_CODE='0' 漏配；只改导入、角色集/policy/状态机/前端零改。app＝全 union（不建 APP allowlist：DSP-only vs union 仅差 6 人；SYSTEM 授予记 system_role_from_user_role 信号+重导入后身份治理页复核）；映射增补 ROLE_SUPER/SUPER_ADMIN→SYSTEM(57人)、TENANT_ADMIN/REFION_ADMIN→ORGAN_MANAGER（已签）。只惠及重导入（守 D62 A1）。签字 .testing/signoff/iam-pub-user-role-materialization.signoff.yaml；全文 docs/decisions/iam-pub-user-role-materialization-D63.md。
