...
zw-brain 的 canonical source 是 `zw_brain/capability_registry/registered/*.json`。
本脚本负责：
...
SKILL_REGISTRY = REPO_ROOT / "zw_brain" / "capability_registry" / "registered"
...
def discover_skills() -> list[dict[str, Any]]:
    ...
    for spec in sorted(SKILL_REGISTRY.glob("*.json")):
        try:
            ...
            skills.append(
                {
                    "skill_id": data.get("slug", spec.stem),
...
# F4: 主入口 slug prefix → product anchor 映射；用 skill_id 的点号前缀派生。
...
WEBUI_PAGE_PREFIX_MAP: list[tuple[str, str]] = [
    ...
    # 主入口归属来自 §5.2 + 业务方反馈，未覆盖的进 UNMAPPED_WEBUI_SKILLS（F5/F7 时复查）。
    ("workbench.", "P1"),
    ("data.search", "P2"),
    ...
    ("registry.", "B1.2"),
    ("org.", "B1.2"),
    # E3 Wave-2 F7：三引擎管理员配置（草稿→预览→入库三步）—— 收编 B1.2 子页 /integration-admin/engines
    ("approval_flow.", "B1.2"),
    ("form_schema.", "B1.2"),
    ("recommendation.", "B1.2"),
    ("system.", "P1"),
...
def build_webui_page_registry(skills: list[dict[str, Any]]) -> str:
    """Generate src/registry/pages.generated.ts —— 把 live skill 按 page anchor 归类。
    ...
    # 输出固定 ordering，让 diff 稳定。三引擎（approval_flow / form_schema / recommendation）收编入 B1.2。
...
def render(
    ...
    lines.append(
        ...
        "符合确定性自动化运营和运维「单一入口、避免堆 N 个独立子命令」原则。"
    )
    lines.append("")
    if cli:
