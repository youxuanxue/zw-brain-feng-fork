...
段 66	scripts/check_credential_grant_invariant.py	credential-honesty-invariant (C-1 凭据诚实化：legacy exchange granted 分支须**显式处理凭据态**——credential + credential_status 都赋值，且**不得**调凭据工厂 derive_platform_credential（原 derive_demo_credential）捏造；真实授权表无 per-grant 凭据→诚实 not_issued。新建在产单 approve 自动签发(平台自签 AK-SELF)不在此约束)
段 67	scripts/check_orphan_rows.py	orphan-rows (数据模型参照完整性脊柱 §六 — 自建干净 seed 库扫 A12+B5+C9 枚举登记父子边孤儿=0 + FK 回潮 floor≥22 + catalog_entry 可达性；A/B 走 M1 FK+CASCADE、C 类 honest 降级由本守卫唯一兜底 design §2.5；覆盖=登记边集合非 schema 全反射)
...
段 77	scripts/check_agent_runtime_import_confinement.py	ar-import-confinement (D68 单一模型 — zw_brain/ 下零 `from agent_runtime` SDK import（ALLOWED_SDK_IMPORTERS 为空，embedded 退役）；防独立 AgentRuntime 服务接入时把 SDK 符号泄漏进业务/entry 层重新紧耦合；上层只调 facade，接缝若需扩面须改 ALLOWED_SDK_IMPORTERS + PR 说明)
段 78	scripts/check_no_zw_brain_inference_env.py	no-zw-brain-inference-env (D68 cleanup — zw-brain runtime 不持有推理平台 SDK/env；模型网关配置只属于独立 AgentRuntime 服务)
