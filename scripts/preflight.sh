...
段 72	scripts/check_d_index_aggregate_size.py	d-index-aggregate-size (全局宪法 §5/§6 净零策展硬化 — 段68 只限单条 D-索引 ≤900 字符、对总量无上限；CLAUDE.md「## 决策记录」段每会话整份入上下文、D1→D61 只增不减=持续稀释烧 token。本守卫给该段加聚合字符棘轮：上限=登记常量 MAX_AGGREGATE_CHARS(实测当前值)，聚合 > 上限即 FAIL，当前 == 上限通过。段边界与段68 check_d_index_entry_size.py 完全一致(同 SECTION_ANCHOR「## 决策记录」锚到 EOF)。语义=加新条须先归档等量老条腾字符；放宽须人工显式改常量、不许靠累积悄悄上调。本守卫只量不改 CLAUDE.md)
