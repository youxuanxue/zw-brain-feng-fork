    ...
    with TemporaryDirectory() as tmp:
        ...
        dash = subprocess.run(
            [
                str(python),
                "-c",
                "import json, os, threading, urllib.request; from http.server import HTTPServer; from sqlalchemy import create_engine; from zw_brain.domain.models import Base; from zw_brain.shared.migrate import ensure_runtime_schema; import zw_brain.command.runtime as runtime; from zw_brain.entry.dashboard_bff import DashboardBffHandler; urllib.request.install_opener(urllib.request.build_opener(urllib.request.ProxyHandler({}))); ensure_runtime_schema(); engine = create_engine(f\"sqlite:///{os.environ['ZW_BRAIN_DB_PATH']}\", future=True); Base.metadata.create_all(bind=engine); server = HTTPServer(('127.0.0.1', 0), DashboardBffHandler); port = server.server_address[1]; thread = threading.Thread(target=server.serve_forever, daemon=True); thread.start(); health = json.loads(urllib.request.urlopen(f'http://127.0.0.1:{port}/health').read().decode()); html = urllib.request.urlopen(f'http://127.0.0.1:{port}/index.html').read().decode(); print(json.dumps({'health': health, 'has_title': '政务数据大脑 · 指挥大屏' in html}, ensure_ascii=False)); server.shutdown(); server.server_close(); thread.join(timeout=2); runtime._service = None",
            ],
            cwd=tmp,
            env=env,
            text=True,
