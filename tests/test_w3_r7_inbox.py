"""W3 业务运营员 P5 工作收件箱 contract test.

Covers:
- catalog.entry.query 新增 source / lifecycle_status 过滤
- 业务运营员 工作流卡片仅 业务运营员 可见（角色权限）
...
- 端到端：提供方部门 反向编目 create → catalog.entry.query(source=reverse, lifecycle=draft)
  返回该草稿 → 业务运营员 confirm → 草稿进入 pending_review，再 query 不返回
"""
...
def test_catalog_entry_query_filter_by_source_and_lifecycle(svc) -> None:
    ...
    result = svc.invoke_skill("catalog.entry.query", {
        "role": "ROLE_BUSIAUDIT", "source": "reverse", "lifecycle_status": "draft",
    })
...
def test_r7_confirm_drops_draft_from_pending_query(svc) -> None:
    """After 业务运营员 confirms, the draft should no longer appear under (reverse, draft)."""
    svc.invoke_skill("catalog.entry.reverse_draft.create", {
        "role": "ROLE_ORGAN_MANAGER", "confirmed": True,
    ...
    pending = svc.invoke_skill("catalog.entry.query", {
        "role": "ROLE_BUSIAUDIT", "source": "reverse", "lifecycle_status": "pending_review",
    })
...
def test_r7_workflow_cards_visible_only_to_r7() -> None:
    ...
    # Cards function references 业务运营员 role check
    assert "r7ProviderWorkflowCards" in pages_js
    assert "window.STATE.role !== 'ROLE_BUSIAUDIT'" in pages_js
