...
const REQUEST_STATUS_PRIORITY = new Map([
  ['draft', 100],
  ['need-fix', 95],
  ['supplementing', 92],
  ['summary-pending', 92],
...
function preferExistingRequest(
  next: Record<string, unknown>,
  current: Record<string, unknown> | undefined,
): boolean {
  if (!current) return true;
  const nextScore = REQUEST_STATUS_PRIORITY.get(String(next.status ?? '')) ?? 20;
  const currentScore = REQUEST_STATUS_PRIORITY.get(String(current.status ?? '')) ?? 20;
