...
const PAGE_MATRIX: Array<{ role: string; hash: string; heading: RegExp | string }> = [
  ...
  { role: 'ROLE_ORGAN_OPERATER', hash: '#/discovery', heading: '找数据' },
  ...
  // 我的申请/授权/交付/需求/异议）；需求/异议子路由保留独立可达并复用同一入口栏。
  { role: 'ROLE_ORGAN_OPERATER', hash: '#/delivery-exchange', heading: '领数据' },
  { role: 'ROLE_ORGAN_OPERATER', hash: '#/request-flow/objection', heading: '领数据' },
  { role: 'ROLE_ORGAN_OPERATER', hash: '#/request-flow/supply-demand', heading: '领数据' },
  ...
  { role: 'ROLE_ORGAN_MANAGER', hash: '#/provider', heading: '供数据' },
  { role: 'ROLE_ORGAN_MANAGER', hash: '#/provider/wizard/reverse-catalog', heading: '反向编目' },
  ...
  { role: 'ROLE_SYSTEM', hash: '#/integration-admin/engines', heading: '流程与表单配置（待接入）' },
];
