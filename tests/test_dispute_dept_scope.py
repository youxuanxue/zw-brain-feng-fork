...
def temp_db() -> Path:
    with TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "dispute_dept_scope.db"
...
def test_dept_scope_keeps_either_side_in_scope(temp_db: Path) -> None:
...
def test_global_role_none_keeps_all(temp_db: Path) -> None:
...
def test_empty_set_fail_closed_keeps_none(temp_db: Path) -> None:
