...
证明：M0 + R1-R8 九个角色都能在 zw-brain 上完成自己的主旅程一次。
每个测试方法对应一个角色，按业务自然流转的顺序串成一条可累积的会话：

    M0 → R1 → R7 → R2 → R6 → R3 → R4 → R5 → R8
...
def _seed_minimum(tenant_id: str = "sd-default") -> None:
    ...
    (so R6 can reverse-catalog), one application (so R2/R5 have something to
    review), one delivery_task with channel=api (so R1's API view has data),
    one delivery_subscription (so R2 撤回处置 has something to terminate)."""
...
class TestAcceptance9Roles:
    ...
    def test_01_m0_acceptance_status_query(self, session) -> None:
        result = session.invoke_skill("legacy.migration.status.query", {"role": "r7"})
    ...
    def test_04_r2_application_review_with_grade_policy(self, session) -> None:
        # R2 P3 reviewDetail 的主审批 skill 是 application.resource.review；
    ...
    def test_08_r5_objection_four_substages(self, session) -> None:
        ...
        # 这是异议状态机的真实路径；验证 R5 能 chain 调主 skill
        session.invoke_skill("objection.case.accept", {
            "role": "r2", "confirmed": True, "objection_id": "OBJ-ACC-001",
        })
        ...
        ev = session.invoke_skill("objection.case.evaluate", {
            "role": "r5", "confirmed": True,
            "objection_id": "OBJ-ACC-001",
            "evaluation_kind": "field-conflict",
            "conclusion": "字段口径冲突，需 R6 修字段证据",
        })
    ...
    def test_09_r8_audit_list_and_direct_access(self, session) -> None:
        audit_list = session.invoke_skill("audit.list", {"role": "r8"})
        assert "items" in audit_list
        # 前面 8 个测试都写了 audit；R8 应该看到至少这些
        assert len(audit_list["items"]) >= 5
        direct = session.invoke_skill("direct_access.delivery.list", {"role": "r8"})
