...
|---|---|---|---|---|
| **Wave 0** | 机械守卫 + 首条黄金链路 | 10 | J1（端到端） + 4 infra | 客户能用真实数据跑通"检索 → 申请 → 审批 → 凭据 → 调用"一次 |
...
| **cross-cutting** | 横切回归 + 角色/消费面/状态机/负向矩阵 | 6 文档 | All | 每 Wave PR 必扫；旧 128 用例映射完成 |
...
- **旧 xlsx 行 N**：`old/共享平台V5.0.2-冒烟.xlsx` 行号 1–128
