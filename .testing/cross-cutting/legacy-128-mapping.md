...
> 旧文件：`old/共享平台V5.0.2-冒烟.xlsx`（128 行）

## 一、统计

| 类别 | 行数 | 占比 |
|---|---|---|
| ✅ 映射到新 .feature（J1/J2/B1/Cross） | 60 | 47% |
| ❌ 不复刻（基线 §5.6 明示 + 业务方反馈） | 50 | 39% |
| ⏸️ 占位延后（国家通道 / Wave 3） | 12 | 9% |
| ⚠️ 外部依赖（集团运维监控 / 消息中心 / IAM） | 6 | 5% |
...
| 行 | 用例名 | 优先级 | 处置 | 映射 |
|---|---|---|---|---|
| 1 | 代理服务资源申请暂存 | 最高 | ❌ 不复刻（代理服务不进 IA，§5.6 #1 #11） | — |
...
| 5 | 融合服务资源申请-有期限 | 最高 | ❌ 融合服务不进 IA（§5.6 #12） | — |
| 6 | 通用服务资源申请暂存 | 中 | ❌ 通用服务不进 IA（§5.6 #12） | — |
| 7 | 通用服务资源申请-有期限 | 中 | ❌ 同上 | — |
...
| 23 | 通用服务列表查看 | 中 | ❌ 同上 | — |
| 24 | 服务申请 | 高 | ❌ "服务"概念合并到 resource.api 物化（§3.3 / §5.6） | wave-1 j2-resource-mount (api 物化) |
| 25 | 首页 | 高 | ✅ 同行 12 | wave-0 j1-resource-discovery |
| 26 | 数购车申请 | 最高 | ❌ "数购车" 是应用中心子能力（§5.6 #11） | — |
...
| 行 | 用例名 | 优先级 | 处置 |
|---|---|---|---|
| 65 / 70 / 63 / 68 / 66 / 67 / 69 / 64 | 创建代理服务 / 通用服务 / 简易融合服务 / 服务审核 / 服务发布等 | 最高 / 高 | ❌ §5.6 #12 融合服务编排 / 通用服务 / 应用案例不进 IA |

### 系统：申请审核（8 行 → J1 主链路变更与续期）

| 行 | 用例名 | 优先级 | 处置 | 映射 |
|---|---|---|---|---|
| 71 | 代理服务申请变更 | 最高 | ❌ 代理服务不进 IA | — |
...
按基线 §5.6 + §3.4 "不进 IA / 外部依赖"，旧 128 用例中以下类型完全不在 wave-N 内：

| 类型 | 不复刻理由 | 旧 128 用例占比 |
|---|---|---|
| 融合服务 / 通用服务 / 代理服务 / 应用中心 / 应用案例 | §5.6 #11 #12 业务反馈 "几乎不用" | 21 行 (16%) |
| 数字化运营 / 大屏 | §1.3 不做大屏 | 4 行 |
| 数据质量检测 / 元数据采集（除 J2 反向编目） | §3.4 集团数据治理中心外部 | 2 行 |
| 主题库 | §5.6 #11 | 3 行 |
| 栏目管理 | 旧门户运营，新平台不重做 | 4 行 |
| 知识中心独立模块 | §5.2.1 合并入 B1.1 | 1 行 |
| 多 SPA 拼接（"访问各子系统"） | §3.5 #6 反模式 | 1 行 |
